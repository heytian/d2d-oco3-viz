...
def update_heatmap(year_range, pop_range, sel_times, sel_seasons, mean_type, layout_mode, city_subset):
    ...
    if layout_mode == "subplot":
        ...
        fig.add_trace(go.Heatmap(
            ...
            colorbar=colorbar_cfg("CO₂ var<br>(ppm)", "+.1f", 1.02, 0.75, TF),
            customdata=cd_co2, hovertemplate=HT_CO2, name="CO₂",
        ), row=1, col=1)

        fig.add_trace(go.Heatmap(
            z=sif_z, x=times, y=cities,
            colorscale="PRGn", zmid=0, zmin=-sif_sym, zmax=sif_sym,
            colorbar=colorbar_cfg("SIF var", "+.3f", 1.02, 0.25, TF),
            customdata=cd_sif, hovertemplate=HT_SIF, name="SIF",
        ), row=2, col=1)
        ...
        for r in [1, 2]:
            fig.update_yaxes(tickfont=TF, autorange="reversed", title=None, row=r, col=1)
        ...
        cd_co2_il = np.full((n_rows, nt, 3), "", dtype=object)
        cd_sif_il = np.full((n_rows, nt, 3), "", dtype=object)

        for i in range(nc):
            z_co2_il[i * 2, :]       = co2_z[i, :]
            z_sif_il[i * 2 + 1, :]   = sif_z[i, :]
            cd_co2_il[i * 2, :, :]   = cd_co2[i, :, :]
            cd_sif_il[i * 2 + 1, :, :] = cd_sif[i, :, :]
        ...
        fig = go.Figure([
            go.Heatmap(
                ...
                colorbar=dict(
                    ...
                    tickformat="+.1f", tickfont=TF,
                    lenmode="fraction", len=0.82, y=0.5, yanchor="middle",
                    x=1.02, xanchor="left", thickness=12, outlinewidth=0,
                ),
                customdata=cd_co2_il, hovertemplate=HT_CO2, name="CO₂",
            ),
            go.Heatmap(
                ...
                colorbar=dict(
                    ...
                    tickformat="+.3f", tickfont=TF,
                    lenmode="fraction", len=0.82, y=0.5, yanchor="middle",
                    x=1.14, xanchor="left", thickness=12, outlinewidth=0,
                ),
                customdata=cd_sif_il, hovertemplate=HT_SIF, name="SIF",
            ),
        ])
